<form id="deleteForm" action="{{ route('bodycam.body-cam.destroy', $data->id) }}" method="post">
    {{ method_field('DELETE') }}
    {{ csrf_field() }}
    <a href="{{ route('bodycam.body-cam.show', $data->id) }}" class="btn btn-primary btn-icon btn-sm"
       data-toggle="tooltip"
       data-placement="top"><i class="bi bi-eye-fill text-white"></i></a>
    <a href="{{ route('bodycam.body-cam.edit', $data->id) }}" class="btn btn-warning btn-icon btn-sm"
