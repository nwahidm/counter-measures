<form id="deleteForm" action="{{ route('close.tailing.pemahaman-perilaku.destroy', $data->id) }}" method="post">
    {{ method_field('DELETE') }}
    {{ csrf_field() }}
    <a href="{{ route('close.tailing.pemahaman-perilaku.show', $data->id) }}" class="btn btn-primary btn-icon btn-sm"
       data-toggle="tooltip"
       data-placement="top"><i class="bi bi-eye-fill text-white"></i></a>
    <a href="{{ route('close.tailing.pemahaman-perilaku.edit', $data->id) }}" class="btn btn-warning btn-icon btn-sm"
