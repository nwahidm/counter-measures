<form id="deleteForm" action="{{ route('close.singleform.single-form.destroy', $data->id) }}" method="post">
    {{ method_field('DELETE') }}
    {{ csrf_field() }}
    <a href="{{ route('close.singleform.single-form.show', $data->id) }}"
    ...
    <a href="{{ route('close.singleform.single-form.edit', $data->id) }}"
