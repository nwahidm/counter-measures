<x-backoffice.layout.app-layout title="Detail Lingkungan Target Penyurupan">
    @push('css')
        <style>
            thead {
                background: #f5f4f8;
    ...
    @endpush
    <x-backoffice.toolbar heading="Detail Lingkungan Target Penyurupan" subheading="" breadcrumb="open-case-detail"
        icon="fas fa-users">
                                    ...
                                    <div class="card ">
                                            ...
                                            <div class="card-toolbar">
                                                <div class="d-flex justify-content-end w-100 gap-2"
                                                    ...
                                                    <button type="button" class="btn btn-dark btn-sm"
                                                        onclick="window.location.href='{{ route('close.intrusion.target-env.download-interview-audio-to-text-file', encrypt($data->id)) }}'">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row mb-5">
                                                ...
                                                <div class="col-lg-8">
                                                    <span
                                                        class="fw-bold fs-6 text-gray-800" style="text-align: justify">{{ strip_tags($data->location?->deskripsi_lokasi) }}</span>
                                                </div>
                                            </div>
                                            <div class="row mb-5">
                                                ...
                                                <div class="col-lg-8">
                                                    @if ($data->location?->lokasi_target_upload)
                                                        <a class="btn btn-dark btn-sm btn-icon"
                                                            href="{{ route('close.intrusion.target-loc.download-file', encrypt($data->location?->lokasi_target_upload)) }}">
                                                    ...
                                                    @if ($data->location?->target_photo)
                                                        @foreach (json_decode($data->location?->target_photo) as $image)
                                                            <img class="image-preview"
                                                                ...
                                                                src="{{ $image ? asset('storage/' . $image) : asset('assets/images/placeholder.jpeg') }}"
                                                                alt="Preview">
                                                        @endforeach
                                                    @endif
                                                </div>
                                            ...
                                            <label class="fs-3 fw-bold">DETAIL LINGKUNGAN TARGET:</label>
                                            <hr>
                                            <div class="row mb-5">
                                                <label class="col-lg-4 fw-semibold text-muted">Nama Lingkungan</label>
                                                <div class="col-lg-8">
                                                    <span
                                                        class="fw-bold fs-6 text-gray-800" style="text-align: justify">{{ strip_tags($data->nama_lingkungan) }}</span>
                                                </div>
                                            </div>
                                            <div class="row mb-5">
                                                ...
                                                <div class="col-lg-8">
                                                    <span
                                                        class="fw-bold fs-6 text-gray-800" style="text-align: justify">{{  strip_tags($data->tipe_lingkungan)}}</span>
                                                </div>
                                            </div>
                                            <div class="row mb-5">
                                                <label class="col-lg-4 fw-semibold text-muted">Deskripsi
                                                    Lingkungan</label>
                                                <div class="col-lg-8">
                                                    <span
                                                        class="fw-bold fs-6 text-gray-800" style="text-align: justify">{{  strip_tags($data->deskripsi_lingkungan) }}</span>
                                                </div>
                                            </div>
                                            <div class="row mb-5">
                                                <label class="col-lg-4 fw-semibold text-muted">Aktivitas
                                                    Teramati</label>
                                                <div class="col-lg-8">
                                                    <span
                                                        class="fw-bold fs-6 text-gray-800" style="text-align: justify">{{  strip_tags($data->aktivitas_teramati) }}</span>
                                                </div>
                                            </div>
                                            <div class="row mb-5">
                                                ...
                                                <div class="col-lg-8">
                                                    <span
                                                        class="fw-bold fs-6 text-gray-800" style="text-align: justify">{{ strip_tags($data->informasi_terkumpul) }}</span>
                                                </div>
                                            </div>
                                            <div class="row mb-5">
                                                <label class="col-lg-4 fw-semibold text-muted mb-3">Download File
                                                ...
                                                <div class="col-lg-8">
                                                    @if ($data->target_environment_upload)
                                                        <iframe style="width: 100%; height: 500px; overflow: hidden;"
                                                            src="{{ asset('storage/' . $data->target_environment_upload) }}"
                                            ...
                                            <label class="fs-3 fw-bold">ANALISA DOKUMEN LINGKUNGAN TARGET:</label>
                                            <hr>
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.device_dahua_id) {
                            ...
                            var fileName = "intrusion_target_env_" + timestamp + "_" + uuid;
                            // Dynamically construct the channelId and playerUrl using JavaScript
        ...
        document.getElementById('stopRecordButton').addEventListener('click', () => {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $.ajax({
                        url: '/close/intrusion/target-env/upload-video',
                        type: 'POST',
                        headers: {
