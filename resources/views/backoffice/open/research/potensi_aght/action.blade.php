<form id="deleteForm" action="{{ route('open.research.tibc.destroy', $data->id) }}" method="post">
    {{ method_field('DELETE') }}
    {{ csrf_field() }}
    @if ($data->caseProgress->percentage != '100') 
    <a href="{{ route('open.research.tibc.edit', $data->id) }}" class="btn btn-warning btn-icon btn-sm"
       data-toggle="tooltip" data-placement="top" title="Edit"><i class="bi bi-pencil-fill text-white"></i></a>

    @endif
    <a href="{{ route('open.research.tibc.show', $data->id) }}" class="btn btn-primary btn-icon btn-sm"
       data-toggle="tooltip"
