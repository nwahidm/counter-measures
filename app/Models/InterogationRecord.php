<?php

namespace App\Models;

use App\Models\User;
use App\Models\OpenCase;
use App\Models\MasterSatker;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\SoftDeletes;
use App\Models\InterogationTargetIdentification;
use App\Models\InterogationResultAchievement;
use Illuminate\Database\Eloquent\Concerns\HasUuids;
use Illuminate\Database\Eloquent\Factories\HasFactory;

class InterogationRecord extends Model
{
    use HasFactory, HasUuids;

    protected $table = 'interrogation_berita_acara';
    protected $guarded = []; 
    protected $primaryKey = 'id_interogation_record';
    protected $casts = [
        'created_at' => 'datetime:Y-m-d H:i:s',
        'updated_at' => 'datetime:Y-m-d H:i:s'
    ];

    public function createdBy()
    {
        return $this->belongsTo(User::class, 'created_by', 'id');
    }

    public function updatedBy()
    {
        return $this->belongsTo(User::class, 'updated_by', 'id');
    }

    public function case()
    {
        return $this->belongsTo(OpenCase::class, 'case_id', 'id');
    }

    public function satker()
    {
        return $this->belongsTo(MasterSatker::class, 'satker_id', 'id_satker');
    }

    public function InteroggTarget()
    {
        return $this->hasMany(InterogationTargetIdentification::class, 'interogation_record_id', 'id_interogation_record');
    }
}
