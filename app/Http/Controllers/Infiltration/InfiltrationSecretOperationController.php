...
use App\DataTables\Infiltration\InfiltrationSecretOperationDataTable;
use App\DataTables\Infiltration\InfiltrationSecretOperationVideoShowDataTable;
...
class InfiltrationSecretOperationController extends Controller
{
    ...
    public function index(InfiltrationSecretOperationDataTable $dataTable)
    {
        ...
        return $dataTable->render('backoffice.close.infiltration.secret-operation.index', compact('satker', 'users'));
    }
    ...
    public function create()
    {
        ...
        return view('backoffice.close.infiltration.secret-operation.create', 
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('backoffice.close.infiltration.secret-operation.edit', 
    ...
    public function update(Request $request, $id)
    {
        $this->validate($request, [
            ...
            'operasi_rahasia_dokumen_upload' => 'nullable|file|mimes:pdf|max:20480',
            'operasi_rahasia_video_upload' => 'nullable|file|mimes:pdf|max:204800'
        ]);
        ...
        if ($request->hasFile('operasi_rahasia_dokumen_upload')) {
            ...
            $document_pdf = new VideoDocuments;
        ...
        if ($data->update()) {
            ...
            return redirect()->route('close.infiltration.secret-operation.index')->with(["success" => "Data berhasil diupdate."]);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($data->save()) {
            ...
            return redirect()->route('close.infiltration.secret-operation.index')->with("success", "Data berhasil ditambah.");
        }

        return redirect()->back()->withInput()->with('error', 'Data gagal disimpan!');
    }

    public function show(Request $request, $secret_operation, InfiltrationSecretOperationVideoShowDataTable $dataTable)
    {
        $data = InfiltrationSecretOperation::find($secret_operation);
        if (!$data) {
            return redirect()->back()->with('error', 'Data tidak ditemukan!');
        }
        ...
        $data->operasi_rahasia_dokumen_upload = Storage::url($data->operasi_rahasia_dokumen_upload);
        $data->operasi_rahasia_video_upload = Storage::url($data->operasi_rahasia_video_upload);

        $bodycam_devices = BodycamDeviceDataHelper::getBodycamDevicebyUser();
        return $dataTable->with('secret_operation', $secret_operation)
            ->render('backoffice.close.infiltration.secret-operation.show', 
            compact(
                'data', 
                'case', 
                'satker', 
    ...
    public function uploadVideo(Request $request)
    {
        ...
        if ($path) {
            ...
            $data_infiltrtion_secret_operation = InfiltrationSecretOperation::where('id', $id)->first();
            $data_infiltrtion_secret_operation->operasi_rahasia_video_upload = $path;
            $data_infiltrtion_secret_operation->update();
    ...
    public function uploadVideo1(Request $request)
    {
        ...
        if ($video) {
            ...
            $path = 'close/infiltration/secret-operation/operasi_rahasia_video_upload/' . $filename;

            $data_interview_hasil = InfiltrationSecretOperation::where('id', $id)->first();
            $data_interview_hasil->operasi_rahasia_video_upload = $path;
    ...
    public function downloadAudiotoTextFile($id)
    {
        ...
        $id = decrypt($id);
        // return $id_case;
        $data = InfiltrationSecretOperation::where('id', $id)->first();
        ...
        $mpdf->WriteHTML(view("backoffice.close.infiltration.secret-operation.pdf", compact(
            'data',
            'images',
            'satker',
