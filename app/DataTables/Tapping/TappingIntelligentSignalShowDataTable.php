...
class TappingIntelligentSignalShowDataTable extends DataTable
{
    /**
    ...
    {
        $id = request()->route('intelligent_signal'); // Asumsi ID diterima dari route
    ...
    {
        $id = request()->route('intelligent_signal'); // Correct parameter name
        ...
        return $this->builder()
            ...
            ->postAjax([
                'url' => route('close.tapping.intelligent_signal.show', ['intelligent_signal' => $id])
            ])
            ->buttons(
    ...
    {
        return 'tapping_intelligent_signal_2-' . date('YmdHis');
    }
}
