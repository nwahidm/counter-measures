...
class IntrusionTargetLocVideoShowDataTable extends DataTable
{
    ...
    {
        $id = request()->route('target_loc'); // Asumsi ID diterima dari route
    ...
    {
        $id = request()->route('target_loc'); // Correct parameter name
        ...
        return $this->builder()
            ...
            ->postAjax([
                'url' => route('close.intrusion.target-loc.show', ['target_loc' => $id])
            ])
            ->buttons(
