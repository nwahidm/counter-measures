...
class IntrusionTargetEnvVideoShowDataTable extends DataTable
{
    ...
    {
        $id = request()->route('target_env'); // Asumsi ID diterima dari route
    ...
    {
        $id = request()->route('target_env'); // Correct parameter name
        ...
        return $this->builder()
            ...
            ->postAjax([
                'url' => route('close.intrusion.target-env.show', ['target_env' => $id])
            ])
            ->buttons(
