...
class InfiltrationTargetDynamicsVideoShowDataTable extends DataTable
{
    ...
    {
        $id = request()->route('target_dynamic'); // Asumsi ID diterima dari route
    ...
    {
        $target_dynamic = request()->route('target_dynamic'); // Ambil parameter dengan benar
        ...
        return $this->builder()
            ...
            ->postAjax([
                'url' => route('close.infiltration.target-dynamics.show', ['target_dynamic' => $target_dynamic])
            ])
            ->buttons(
