...
class InfiltrationSecretOperationVideoShowDataTable extends DataTable
{
    ...
    {
        $id = request()->route('secret_operation'); // Asumsi ID diterima dari route
    ...
    {
        $secret_operation = request()->route('secret_operation'); // Ambil parameter dengan benar
        ...
        return $this->builder()
            ...
            ->postAjax([
                'url' => route('close.infiltration.secret-operation.show', ['secret_operation' => $secret_operation])
            ])
            ->buttons(
