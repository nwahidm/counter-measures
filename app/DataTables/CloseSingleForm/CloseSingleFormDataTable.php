...
class CloseSingleFormDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('case_procedure_type', function ($data) {
                return Str::limit(strip_tags($data->close_procedure_type), 128, '');
            })
            ...
            ->editColumn('action', function ($data) {
                return view('backoffice.close.single-form.action', compact('data'))->render();
            })
            ->rawColumns([
     ...
     */
    public function query(CloseCaseSingleForm $model)
    {
        $user = auth()->user();
        $satker = $user->satker;
        ...
        if($user->user_roles == "superadmin"){
            ...
            return $model->newQuery()->where('satker_id', 'like', "$idSatker%");
    ...
    public function html()
    {
        ...
        return $this->builder()
            ...
            ->postAjax([
                'url' => route('close.singleform.single-form.index')
            ])
            ->buttons(
