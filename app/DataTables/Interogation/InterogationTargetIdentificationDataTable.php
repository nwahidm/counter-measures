...
class InterogationTargetIdentificationDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->editColumn('target_name', function ($data) {
                // return json_encode($data->interogreord);
            ...
            ->editColumn('action', function ($data) {
                if ($data->caseProgress != null && $data->caseProgress->percentage < 100) {
                    return view('backoffice.open.interogation-target-identification.action', compact('data'))->render();
                } else {
                    return view('backoffice.open.interogation-target-identification.action-completed', compact('data'))->render();
                }
                
            })
            ->rawColumns([
                'action', 'hasil_target_identification'
            ]);
    }
     ...
     */
    public function query(InterogationTargetIdentification $model): QueryBuilder
    {
        $user = auth()->user();
        $idSatker = $user->satker->id_satker;
        return $model->newQuery()
        ->with(['interogRecord', 'case', 'satker'])
        ->when(
        ...
        ->orderby('interrogation_identifikasi_target.created_at','DESC');;
    }
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
            ...
            ->postAjax([
                'url' => route('open.data.interogg-target-id.index')
            ])
            ->buttons(
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('target_name')
                ->name('interogRecord.target_name')
                ->title('NAMA TARGET')
                ->className('text-center')
                ->footer('NAMA TARGET'),
            Column::make('kasus')
                ...
                ->className('text-center')
                ->footer('KASUS'),
            Column::make('hasil_target_identification')
                ->name('hasil_target_identification')
                ->title('HASIL DICAPAI')
                ->className('text-center')
    ...
    {
        return 'InterogationTargetIdentification_' . date('YmdHis');
    }
}
